...
console.log('Has Kalshi key:', !!KALSHI_API_KEY);
...
async function main() {
  ...
  // Find orders that need to be recovered
  ...
  for (const [orderId, fills] of fillsByOrderId) {
    ...
    const avgPrice = fills.reduce((sum, f) => sum + (f.price || 0) * (f.count || 0), 0) / totalCount;
  ...
  for (const [batchDate, orders] of ordersByDate) {
    ...
    for (const order of orders) {
      ...
      if (insertError) {
        console.error(`  Failed to insert ${order.ticker}:`, insertError.message);
      } else {
        totalRecovered++;
        console.log(`  ✓ ${order.ticker} (${order.side}) - ${order.result_status}`);
      }
    }
  ...
  console.log(`\n✓ Recovery complete! Recovered ${totalRecovered} orders.`);
}
